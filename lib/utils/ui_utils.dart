...
import 'package:Tijaraa/ui/screens/widgets/full_screen_image_view.dart';
import 'package:Tijaraa/ui/screens/widgets/gallery_view.dart';
...
class UiUtils {
  ...
  static String convertToAgo({
    ...
    final dateTime = DateTime.tryParse(setDate);
    if (dateTime == null) return setDate;

    final today = DateTime.now();
    final diff = today.difference(dateTime);

    final agoLabel = 'ago'.translate(context);
    final days = diff.inDays;

    if (days >= 365) {
      final year = diff.inDays ~/ 365;
      return '$year ${year > 1 ? 'years'.translate(context) : 'year'.translate(context)} $agoLabel';
    } else if (days >= 30) {
      final month = diff.inDays ~/ 30;
      return '$month ${month > 1 ? 'months'.translate(context) : 'month'.translate(context)} $agoLabel';
    } else if (days > 1) {
      return '$days ${'days'.translate(context)} $agoLabel';
    } else if (days == 1) {
      return 'today'.translate(context);
    } else {
      if (diff.inHours > 0) {
        return '${diff.inHours} ${diff.inHours > 1 ? 'hours'.translate(context) : 'hour'.translate(context)} $agoLabel';
      } else if (diff.inMinutes > 0) {
        return '${diff.inMinutes} ${diff.inMinutes > 1 ? 'minutes'.translate(context) : 'minute'.translate(context)} $agoLabel';
      } else {
